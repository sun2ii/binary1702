...
export const db = {
   ...
   */
  async createProfile({ slug, firstName, lastName, businessName, phone, email, website, linkedin_handle, github_handle, occupation }) {
    const result = await sql`
      INSERT INTO profiles (
        slug, first_name, last_name, business_name, phone, email, website, linkedin_handle, github_handle, occupation, status
      )
      VALUES (
        ${slug}, ${firstName}, ${lastName}, ${businessName || null}, ${phone || null}, ${email}, ${website || null},
        ${linkedin_handle || null}, ${github_handle || null}, ${occupation || null}, 'pending_verification'
      )
  ...
  async getProfileBySlug(slug) {
    const result = await sql`
      SELECT id, slug, first_name, last_name, business_name, phone, email, website, linkedin_handle, github_handle, occupation,
             status, verified_at
      FROM profiles

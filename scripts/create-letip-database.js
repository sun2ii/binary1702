/**
 * Script to create the LeTip Client Leads Notion database
 *
 * Usage: node scripts/create-letip-database.js
 *
 * Requires:
 * - NOTION_TOKEN env var
 * - A parent page ID where the database will be created
 */
...
// Get parent page ID from command line or use default workspace
const parentPageId = process.argv[2];

if (!parentPageId) {
  console.error('Usage: node scripts/create-letip-database.js <parent-page-id>');
  console.error('\nTo find your parent page ID:');
  console.error('1. Open the Notion page where you want to create the database');
  console.error('2. Copy the URL - it looks like: https://www.notion.so/workspace/PAGE_ID?...');
  console.error('3. Extract the PAGE_ID (32 character alphanumeric string)');
  console.error('4. Run: node scripts/create-letip-database.js PAGE_ID');
  process.exit(1);
}
...
createDatabase(parentPageId)
  .then(database => {
    ...
    console.log('2. Share the database with your Notion integration');
